...
public class CustomSecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        ...
        http.csrf().disable();
        //rememberMe: 자동 로그인
        //key(): 인증받은 사용자의 정보로 token 생성, key값은 임의로 설정
        //tokenRepository(): rememberMe의 토큰 저장소
        //userDetailsService(): 유저의 정보를 가져오는 인터페이스
        //tokenValiditySeconds(): 생성된 token의 만료시간
        http.rememberMe().key("12345678").tokenRepository(persistentTokenRepository())
                .userDetailsService(customUserDetailsService).tokenValiditySeconds(60*60*24*30);
    ...
    //정식 사용지가 아닌데 게시글 수정하려고 접근했을 때 서버에서 접근 거부하면 예외처리함
    @Bean

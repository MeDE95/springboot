...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    public OAuth2User loadUser(OAuth2UserRequest userRequest) throws OAuth2AuthenticationException {
        ...
        log.info("oauth2 사용자 ******************");
        //ClientRegistration: OAuth2 제공자에 등록된 클라이언트 정보를 나타내는 클래스
        ...
        //OAuth2User에서 얻어온 속성 정보를 Map 배열형태로 paramMap에 저장
    ...
    //카카오 서비스에서 얻어노 이메일을 이용해 같은 이메일을 가진 사용자가 없으면 자동으로 
    //회원가입하고 MemberSecurity을 반환
    private MemberSecurityDTO generatedDTO(String email, Map<String, Object> params) {
        ...
        if (result.isEmpty()) {
            ...
            Member member=Member.builder().mid(email)
            ...
            //GrantedAuthority: 현재 사용자가 가지고 있는 권한. ROLE_USER 형태
            MemberSecurityDTO memberSecurityDTO = new MemberSecurityDTO(
                    email,"1111", email, false,true, Arrays
                    .asList(new SimpleGrantedAuthority("ROLE_USER")));
            //memberSecurityDTO의 prop에 params 값 세팅
            memberSecurityDTO.setProps(params);
            return memberSecurityDTO;
            //result 값이 비어있지 않다면(이미 회원이라면)
        } else {
    ...
    private String getKakaoEmail(Map<String, Object> paramMap) {
        ...
        log.info(value);
        //LinkedHashMap: Map에 입력된 순서를 기억하는 자료구조

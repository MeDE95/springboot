...
public class Custom403handler implements AccessDeniedHandler {
    ...
    public void handle(HttpServletRequest request, HttpServletResponse response, AccessDeniedException
        ...
        log.info("--------서버 요청 거부--------");
        //응답코드 지정(Http 상태를 접근거부(403) 상태로 응답코드 지정)
        response.setStatus(HttpStatus.FORBIDDEN.value());
        //서버에 요청하는 메시지의 header 중 Content-Type 값을 contentType 변수에 문자열로 저장, JSON 요청이 있는지 확인
        String contentType=request.getHeader("Content-Type");
        //startsWith: 비교대상 문자열이 입력된 문자열로 시작하는지 여부를 확인하고 true/false로 결과 return
